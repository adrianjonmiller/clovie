...
export class Compile extends ServiceProvider {
    ...
    return {
      ...
      views: async (opts) => {
        ...
        for (let i = 0; i < filePaths.length; i++) {
          ...
          promises.push(
            this.#template(filePath, opts).then(() => {
              this.#progressTracker.updateProgress(progressId, i + 1, { 
                current: path.basename(filePath) 
              });
            })
      ...
      template: (path, opts) => this.#template(path, opts),
  ...
  async #route(route, opts, item, key) {
    ...
    if (opts.templateCompiler) {
      ...
      let renderedContent = opts.templateCompiler(content, data);
  ...
  async #asset(filePath, opts) {
    ...
    if (content) {
      ...
      this.#registerOutputPath(filePath, outputPath, opts.outputDir);
  ...
  #loadPartial(filePath, opts) {
    ...
    if (content && opts.templateRegister && typeof opts.templateRegister === 'function') {
      try {
        ...
        const templateRegister = opts.templateRegister || opts.register;
  ...
  async #template(filePath, opts) {
    const [file, liveReload, log] = this.useContext('file', 'liveReload', 'log');
    const content = file.read(filePath);

    if (content && opts.templateCompiler && typeof opts.templateCompiler === 'function') {
      const fileName = path.basename(filePath);
      ...
      try {
        ...
        // Compile template with data
        let renderedContent = opts.templateCompiler(content, opts.data);
