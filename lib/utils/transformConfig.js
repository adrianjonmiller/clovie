...
export async function transformConfig(config, log = null) {
  ...
  if (discovered.watch === undefined) {
    // Only auto-enable watch in development mode, not based on command
    discovered.watch = discovered.mode === 'development';
  }
  ...
  if (discovered.open === undefined) {
    const hasPages = !!(discovered.views || discovered.routes?.length);
    discovered.open = hasPages;
  }
  ...
  // Development mode detection - only if not explicitly set
  ...
  if (!discovered.renderEngine) {
    discovered.renderEngine = await loadRenderEngine('nunjucks', discovered);  
  } else if (typeof discovered.renderEngine === 'string') {
    discovered.renderEngine = await loadRenderEngine(discovered.renderEngine, discovered);  
  }
