...
export async function discoverProjectStructure(config) {
  ...
  if (!discovered.styleCompiler) {
    let styleFiles = [];
    if (discovered.styles) {
      // Handle both absolute and relative paths, and both files and directories
      const stylesPath = path.isAbsolute(discovered.styles) 
        ? discovered.styles 
        : path.join(cwd, discovered.styles);
      
      try {
        const stats = fs.statSync(stylesPath);
        if (stats.isDirectory()) {
          styleFiles = fs.readdirSync(stylesPath);
        } else if (stats.isFile()) {
          // If it's a file, get files from its directory
          const dir = path.dirname(stylesPath);
          styleFiles = fs.readdirSync(dir);
        }
      } catch (err) {
        // Path doesn't exist, continue with empty array
        styleFiles = [];
      }
    }
  ...
  if (!discovered.scriptCompiler) {
    ...
    if (discovered.scripts) {
      // Handle both absolute and relative paths, and both files and directories
      const scriptsPath = path.isAbsolute(discovered.scripts) 
        ? discovered.scripts 
        : path.join(cwd, discovered.scripts);
      
      try {
        const stats = fs.statSync(scriptsPath);
        if (stats.isDirectory()) {
          scriptFiles = fs.readdirSync(scriptsPath);
        } else if (stats.isFile()) {
          // If it's a file, get files from its directory
          const dir = path.dirname(scriptsPath);
          scriptFiles = fs.readdirSync(dir);
        }
      } catch (err) {
        // Path doesn't exist, continue with empty array
        scriptFiles = [];
