...
import { replaceRouteParams } from '../utils/routeMatch.js';
import { formatOutputPath } from '../utils/outputPath.js';
import { isFactory } from '../utils/normalizeToFactories.js';
...
export class Compile extends ServiceProvider {
  ...
  actions(useContext) {
    ...
    return {
      ...
      scripts: async (opts) => {
        ...
        const esBuild = useContext('esBuild');
        if (!esBuild) {
          log.warn('No esbuild compiler available for scripts');
          return;
        }
      ...
      views: async (opts) => {
        ...
        for (let i = 0; i < filePaths.length; i++) {
          const filePath = filePaths[i];
          promises.push(
            this.#view(filePath, opts).then((content) => {
              this.#progressTracker.updateProgress(progressId, i + 1, { 
                current: path.basename(filePath) 
              });
              return content;
      ...
      routes: async (opts) => {
        ...
        const routeConfigs = opts.routes.filter(r => !isFactory(r));
        if (!routeConfigs.length) {
          log.debug('No routes to compile');
          return [];
        }
        
        const progressId = this.#progressTracker.createProgressBar('Compiling Routes', routeConfigs.length, 'routes');
        const promises = [];
        const routesLength = routeConfigs.length;
        ...
        for (let i = 0; i < routesLength; i++) {
          ...
          if (route.repeat) {
            ...
            for (const [key, value] of entries) {
              promises.push(
                this.#route(route, opts, value, key).then((content) => {
                  this.#progressTracker.updateProgress(progressId, i + 1, { 
          ...
          promises.push(
            this.#route(route, opts, null, null).then((content) => {
              this.#progressTracker.updateProgress(progressId, i + 1, { 
                current: route.path || 'route' 
              });
              return content;
        ...
        const results = await Promise.all(promises);
        this.#progressTracker.completeProgress(progressId, `✅ Compiled ${routeConfigs.length} routes`);
      ...
      view:     (filePath, opts) => this.#view(filePath, opts),
  ...
  async #view(filePath, opts) {
    ...
    if (file.exists(filePath) && opts.renderEngine) {
      ...
      this.#write(relativePath, renderedContent, opts.outputDir);
  ...
  async #route(route, opts, value, key) {
    ...
    if (file.exists(route.template) && opts.renderEngine) {
      ...
      this.#write(outputPath, renderedContent, opts.outputDir);
      return renderedContent;
    } else {
  ...
  #write(filePath, content, outputDir) {
    const [file, fileCache, log, liveReload] = this.useContext('file', 'fileCache', 'log', 'liveReload');
    const outputPath = path.join(outputDir, filePath);
        
    // Cache based on OUTPUT content (handles both template and data changes)
    if (fileCache.hasChanged(outputPath, content)) {
