...
import { resolveRoutes } from '../utils/viewsToRoutes.js';
...
export class Compile extends ServiceProvider {
  ...
  async #asset(filePath, opts) {
    const [file, log] = this.inject('file', 'log');
  ...
  async #view(filePath, opts) {
    const [file, liveReload, log] = this.inject('file', 'liveReload', 'log');
  ...
  async #injectLiveReloadScript(renderedContent, opts) {
    const [liveReload, log] = this.inject('liveReload', 'log');
    try {
  ...
  #write(filePath, content, outputDir) {
    const [file, fileCache, log, liveReload] = this.inject('file', 'fileCache', 'log', 'liveReload');
