...
import { normalizeToFactories, normalizeRoutesToFactories } from '../utils/normalizeToFactories.js';
import { viewsToRoutes } from '../utils/viewsToRoutes.js';
...
export class Run extends ServiceProvider {
  ...
  async #buildScripts(opts, htmlStrings, compile, log) {
    ...
    const esBuild = this.useContext('esBuild');
    if (!esBuild) return;
  ...
  async #serve (opts) {
    ...
    const factories = [
      ...
      ...normalizeRoutesToFactories(opts.routes, defineRoutes, opts, services),
      ...normalizeToFactories(viewRoutes, defineRoutes),
    ];
