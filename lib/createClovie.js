...
import { EsBuild } from '@jucie.io/engine-esbuild';
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      clovie.configurator.onReady(async (opts) => {
        ...
        if (opts.scripts) {
          clovie.install(EsBuild.configure({
        ...
        opts.api = opts.api && opts.api.length ? resolveRoutes(opts.api, (...args) => clovie.useContext(...args)) : [];
        opts.routes = opts.routes && opts.routes.length ? resolveRoutes(opts.routes, (...args) => clovie.useContext(...args)) : [];
